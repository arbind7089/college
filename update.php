...
if(@$_GET['fdid'] && $_SESSION['key']=='admin7785068889') {
$id=@$_GET['fdid'];
$result = mysqli_query($con,"DELETE FROM feedback WHERE id='$id' ") or die('Error');
header("location:dashboard.php?admin=3");
}
}

//delete user
if(isset($_SESSION['key'])){
if(@$_GET['demail'] && $_SESSION['key']=='admin7785068889') {
$demail=@$_GET['demail'];
...
//remove quiz
if(isset($_SESSION['key'])){
if(@$_GET['admin']== 'rmquiz' && $_SESSION['key']=='admin7785068889') {
$eid=@$_GET['eid'];
...
header("location:dashboard.php?admin=5");
}
}
...
print_r($_SESSION);
if(isset($_SESSION['key'])){
if(@$_GET['admin']== 'addquiz' && $_SESSION['key']=='admin7785068889') {
$name = $_POST['name'];
...
header("location:dashboard.php?admin=4&step=2&eid=$id&n=$total");
}
}

//add question
if(isset($_SESSION['key'])){
if(@$_GET['admin']== 'addqns' && $_SESSION['key']=='admin7785068889') {
$n=@$_GET['n'];
$eid=@$_GET['eid'];
$ch=@$_GET['ch'];
...
{
	$sn++;
	header("location:tests.php?testId=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
}
else if( $_SESSION['key']!='admin7785068889')
{
...
header("location:tests.php?testId=result&eid=$eid");
}
else
{
header("location:tests.php?testId=result&eid=$eid");
...
//restart quiz
if(@$_GET['testId']== 'quizre' && @$_GET['step']== 25 ) {
$eid=@$_GET['eid'];
$n=@$_GET['n'];
$t=@$_GET['t'];
$q=mysqli_query($con,"SELECT score FROM history WHERE eid='$eid' AND email='$email'" )or die('Error156');
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:tests.php?testId=quiz&step=2&eid=$eid&n=1&t=$t");
}
